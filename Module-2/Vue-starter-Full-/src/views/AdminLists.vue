...
export default {
  ...
  methods: {
    ...
    deleteAdmin() {
      console.log(this.id);
      this.$store.dispatch("DELETE_ADMIN", this.id).then((res) => {
        if (res) {
  ...
  computed: {
    getAdmins() {
      console.log(this.$store.state.adminActions.adminList);
      return this.$store.state.adminActions.adminList;
    },
  },
};
</script>
